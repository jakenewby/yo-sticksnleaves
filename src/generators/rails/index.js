...
module.exports = Generators.Base.extend({
   ...
   */
   ...
   _copyRubocop: function() {
     return new Promise(function(resolve, reject) {
       this.spawnCommand('mkdir', ['rubocop'])
         .on('exit', function(err) {
           if (err) {
             reject(err);
           } else {
       ...
       this.fs.copyTpl(
         this.templatePath('enabled.yml'),
         this.destinationPath('rubocop/enabled.yml')
        );

        this.fs.copyTpl(
          this.templatePath('disabled.yml'),
          this.destinationPath('rubocop/disabled.yml')
        );
